...
public class DirectPreviewAPK
{
	...
	public static void InstallSimulatorInner()
	{
		try
		{
			Process myProcess = new Process();
			myProcess.StartInfo.FileName = "C:\\Windows\\system32\\cmd.exe";
			myProcess.StartInfo.Arguments = "/c adb install -r -d Assets/Plugins/wvr_plugins_directpreview_agent.apk";
			myProcess.Start();
			myProcess.WaitForExit();
			int ExitCode = myProcess.ExitCode;
			if (ExitCode == 0)
	...
	public static void StartSimulatorInner()
	{
		try
		{
			Process myProcess = new Process();
			myProcess.StartInfo.FileName = "C:\\Windows\\system32\\cmd.exe";
			myProcess.StartInfo.Arguments = "/c adb shell am start com.vive.rrclient/.RRClient";
			myProcess.Start();
			myProcess.WaitForExit();
			int ExitCode = myProcess.ExitCode;
			if (ExitCode == 0)
	...
	public static void KillSimulatorInner()
	{
		try
		{
			Process myProcess = new Process();
			myProcess.StartInfo.FileName = "C:\\Windows\\system32\\cmd.exe";
			myProcess.StartInfo.Arguments = "/c adb shell am kill com.vive.rrclient";
			myProcess.Start();
			myProcess.WaitForExit();
			int ExitCode = myProcess.ExitCode;
			if (ExitCode == 0)
	...
	public static void StopSimulatorInner()
	{
		try
		{
			Process myProcess = new Process();
			myProcess.StartInfo.FileName = "C:\\Windows\\system32\\cmd.exe";
			myProcess.StartInfo.Arguments = "/c adb shell am force-stop com.vive.rrclient";
			myProcess.Start();
			myProcess.WaitForExit();
			int ExitCode = myProcess.ExitCode;
			if (ExitCode == 0)
