	...
	void Update()
	{
		...
		for (uint i = 0; i < 2; i++)
		{
			foreach (WVR_InputId buttonId in buttonIds)
			{
				...
				{
					Log.d (LOG_TAG, "device " + device + ", button " + buttonId + " press down");
				...
				{
					Log.d (LOG_TAG, "device " + device + ", button " + buttonId + " press up");
				...
				{
					Log.d (LOG_TAG, "device " + device + ", button " + buttonId + " pressed.");
				}
			}
			...
			foreach (WVR_InputId axisId in axisIds)
			{
				...
				{
					Log.d (LOG_TAG, "device " + device + ", button " + axisId + " touch down.");
				...
				{
					Log.d (LOG_TAG, "device " + device + ", button " + axisId + " touch up.");
				...
				if (WaveVR_Controller.Input (device).GetTouch (axisId))
				{
					...
					Log.d(LOG_TAG, "axis: " + axis);
					float xangle = 360 * axis.x, yangle = 360 * axis.y;
					Log.d (LOG_TAG, "device " + device + ", button " + axisId + " axis xangle: " + xangle + ", yangle: " + yangle);
		...
		if (controller_rotates)
		{
			...
			//ControlledObject.transform.localPosition = this.transform.position + WaveVR_Controller.Input(device).transform.pos;
		}
	} // Update
}
